        <footer id="footer">
            <div class="footer-container max-width">
                <div class="footer-left">
                    
                    <div class="footer-adress">
                        <div>
                            <h4>Consultations</h4>
                            <p>15, Rue Lombard</p>
                            <p>1205 Genève</p>
                            <a href="https://maps.app.goo.gl/338kTmaU3WRHRvHM6" class="underline-link" target="_blank">Voir sur Google Maps</a>
                            
                        </div>
                        <div>
                            <h4>Traitements de PMA</h4>
                            <p>BabyImpulse - Clinique des Grangettes</p>
                            <p>Chemin des Grangettes 7</p>
                            <p>1224 Chêne Bougeries</p>
                            <a href="https://maps.app.goo.gl/JE6D6DEYLwPxookW6" class="underline-link" target="_blank">Voir sur Google Maps</a>
                        </div>
                    </div>
                    <a class="footer-left-logo" href="/">
                        <img src="../img/logo.png" alt="Logo">
                        <h3>Grace Bianchi</h3>
                    </a>
                    <p>Spécialiste FMH, Médecine de la reproduction, Gynécologie-Obstétrique</p>
                    <a href="tel:0223478646" class="underline-link">022 347 86 46</a>
                    <div class="footer-socials">
                        <a href="https://www.linkedin.com/in/grace-bianchi-67b26290/" target="_blank">
                            <!-- <img src="./img/icons/linkedin.svg" alt="LinkedIn"> -->
                            <svg height="28px" width="28px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                            viewBox="-143 145 512 512" xml:space="preserve">
                                <path d="M329,145h-432c-22.1,0-40,17.9-40,40v432c0,22.1,17.9,40,40,40h432c22.1,0,40-17.9,40-40V185C369,162.9,351.1,145,329,145z
                                M41.4,508.1H-8.5V348.4h49.9V508.1z M15.1,328.4h-0.4c-18.1,0-29.8-12.2-29.8-27.7c0-15.8,12.1-27.7,30.5-27.7
                                c18.4,0,29.7,11.9,30.1,27.7C45.6,316.1,33.9,328.4,15.1,328.4z M241,508.1h-56.6v-82.6c0-21.6-8.8-36.4-28.3-36.4
                                c-14.9,0-23.2,10-27,19.6c-1.4,3.4-1.2,8.2-1.2,13.1v86.3H71.8c0,0,0.7-146.4,0-159.7h56.1v25.1c3.3-11,21.2-26.6,49.8-26.6
                                c35.5,0,63.3,23,63.3,72.4V508.1z"/>
                            </svg>
                        </a>
                    </div>
                </div>
                <div class="footer-right">
                    <div>
                        <h4>Navigation</h4>
                        <ul>
                            <li><a class="underline-link" href="/">Accueil</a></li>
                            <li><a class="underline-link" href="/about">Présentation</a></li>
                            <li><a class="underline-link" href="/contact">Contact</a></li>
                            <li><a class="underline-link" href="/config">Admin</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4>Liens</h4>
                        <ul>
                            <li><a class="underline-link" href="https://babyimpulse.ch/" target="_blank">Babyimpulse</a></li>
                            <li><a class="underline-link" href="#">Conditions</a></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="footer-copyright">
                <p>© 2024-<?php echo date("Y"); ?> | Dre P. Grace Bianchi Movarekhi</p>
                <p>Site web réalisé par <a class="underline-link" target=(_blank) href="https://thbo.ch">THBO</a></p>
            </div>
        </footer>
        <script src="../js/index.js?v=4.0.1" defer></script>
        <script src="../js/thbo-parallax.js?v=4.0.1" defer></script>
        <script src="https://thbo.ch/credits.js"></script>
    </body>
</html>